...
@zedub.zed_cmd(pattern=f"تتحميل$")
async def _(event):
    if event.fwd_from:
        return
...
@zedub.zed_cmd(pattern=f"العد التنازلي$")
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 1
