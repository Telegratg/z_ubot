...
async def who(event):
    ...
    if replied_user.id in zel_dev:
        return await edit_or_reply(
            zed, "**- دي . . انـهُ احـد المطـورين . . انتـه الحيـوان ولك**"
        )
    if (
        replied_user.id == 925972505
        or replied_user.id == 1895219306
        or replied_user.id == 2095357462
    ):
        return await edit_or_reply(
            zed, "**- دي . . انـهُ المطـور . . انتـه الحيـوان ولك**"
        )
