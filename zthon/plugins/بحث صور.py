...
async def _(event):
    ...
    async with aiohttp.ClientSession() as requests:
        data = {"q": zedthon, "app_id": ZELZAL_APP_ID, "p": "GoogleImages"}
        reponse = await requests.get(input_url, params=data, headers=headers)
        ...
        for result in response["results"]:
            ...
            image_save_path = os.path.join(wzed_dir, image_file_name)
    ...
    if not url_lst:
        ...
        return
    await event.reply(file=url_lst, parse_mode="html", force_document=True)
    ...
    await event.edit(
        f"**- اكتمـل البحث عـن {zedthon} في {ms} ثانيـه ✓**", link_preview=False
    )
