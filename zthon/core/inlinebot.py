...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**ZThon"):
            buttons = [
                (
                    ...
                    Button.url("𝘼𝙇𝙍𝘼𝘿𝙏𝙃𝙀𝙉 ⩫™ 𓅛", "https://t.me/ALRADTHEN"),
                )
            ]
            ALIVE_PIC = (
            ...
            result = builder.article(
                title="رسـاله سـࢪيـه",
                text=f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝘼𝙇𝙍𝘼𝘿𝙏𝙃𝙀𝙉 ⩫ **- ࢪسـالة سـࢪيـه 🗳**\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n**⌔╎الࢪسـاله لـ** {zilzal} \n**⌔╎هو فقط من يستطيع فتحهـا**",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            result = builder.article(
                title="** همسـه سـريـه**",
                text=f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝘼𝙇𝙍𝘼𝘿𝙏𝙃𝙀𝙉 ⩫ **- همسـة سـࢪيـه 📠**\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n**⌔╎الهمسـة لـ** {zilzal} \n**⌔╎هو فقط من يستطيع ࢪؤيتهـا**",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            result = builder.article(
                title="**⌔╎ࢪسـاله مشفـࢪه 📟**",
                text=f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝘼𝙇𝙍𝘼𝘿𝙏𝙃𝙀𝙉 ⩫ **- ࢪسـالة مشفـࢪه 📟**\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n**⌔╎اضغـط الـزࢪ لـ إظهـاࢪ الࢪسـاله**",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
        ...
        text, msg_entities = await event.client._parse_message_text(
            "- لـ تنصيب سـورس زدثــون 𝘼𝙇𝙍𝘼𝘿𝙏𝙃𝙀𝙉 ⩫", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
